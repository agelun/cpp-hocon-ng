...
namespace hocon {
    ...
    shared_object config_delayed_merge_object::with_value(path, shared_value) const {
        throw not_resolved();
    }

    shared_object config_delayed_merge_object::with_value(string, shared_value) const {
        throw not_resolved();
    }
    ...
    shared_value config_delayed_merge_object::attempt_peek_with_partial_resolve(string const& key) const {
        ...
        for (auto&& layer : _stack) {
            if (auto object_layer = dynamic_pointer_cast<const config_object>(layer)) {
                ...
                throw not_resolved_exception(string_format("Key '%s' is not available at '%s' because value at '%s' has not been resolved and may turn out to contain or hide '%s'. Be sure to config::resolve() before using a config object", key.c_str(), origin()->description().c_str(), layer->origin()->description().c_str(), key.c_str()));
                ...
                if (!dynamic_pointer_cast<const simple_config_list>(layer)) {
                    throw bug_or_broken_exception(string_format("Expecting a list here, not %s", layer->render().c_str()));
    ...
    shared_object config_delayed_merge_object::without_path(path) const {
        throw not_resolved();
    }

    shared_object config_delayed_merge_object::with_only_path(path) const {
        throw not_resolved();
    }

    shared_object config_delayed_merge_object::with_only_path_or_null(path) const {
        throw not_resolved();
    }
