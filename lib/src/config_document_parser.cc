...
namespace hocon { namespace config_document_parser {
    ...
    shared_token parse_context::next_token() {
        ...
        if (_flavor == config_syntax::JSON) {
            if (t->get_token_type() == token_type::UNQUOTED_TEXT && !is_unquoted_whitespace(t)) {
                throw parse_error(string_format("Token not allowed in valid JSON: '%s'", t->token_text().c_str()));
                                                    ...
                                                    path *last_path)
    {
        ...
        if (bad_token == tokens::end_token()->to_string()) {
            ...
            if (!prev_field_name.empty()) {
                part = string_format("%s (if you intended '%s' to be part of a value, instead of a key, try adding double quotes around the whole value", message.c_str(), prev_field_name.c_str());
            } else {
                return message;
            }
        } else {
            if (!prev_field_name.empty()) {
                part = string_format("%s (if you intended %s to be part of the value for '%s', try enclosing the value in double quotes",  message.c_str(), bad_token.c_str(), prev_field_name.c_str());
            } else {
                part = string_format("%s (if you intended %s to be part of a key or string value, try enclosing the key or value in double quotes", message.c_str(), bad_token.c_str());
            }
        }

        if (inside_equals) {
            return string_format("%s, or you may be able to rename the file .properties rather than .conf)", part.c_str());
        } else {
            return part + ")";
        }
    }
    ...
    shared_node_value parse_context::parse_value(shared_token t) {
        ...
        if (t->get_token_type() == token_type::VALUE || t->get_token_type() == token_type::UNQUOTED_TEXT ||
            ...
            throw parse_error(add_quote_suggestion(t->to_string(),
                                                   string_format("Expecting a value but got wrong token: %s", t->to_string().c_str())));
        }
    ...
    shared_ptr<config_node_path> parse_context::parse_key(shared_token token) {
        if (_flavor == config_syntax::JSON) {
            if (tokens::is_value_with_type(token, config_value::type::STRING)) {
                ...
                throw parse_error(string_format("Expecting close brace } or a field name here, got %s", token->to_string().c_str()));
            }
        } else {
            ...
            if (expression.empty()) {
                throw parse_error(string_format("expecting a close brace or a field name here, got %s", t->to_string().c_str()));
            }
    ...
    shared_ptr<config_node_include> parse_context::parse_include(shared_node_list& children) {
        ...
        if (t->get_token_type() == token_type::UNQUOTED_TEXT) {
            ...
            if (kind_text == "url(") {
                ...
                throw parse_error(string_format("expecting include parameter to be quoted filename, file(), classpath(), or url(). No spaces are allowed before the open paren. Not expecting: %s", t->to_string().c_str()));
            }
            ...
            if (!tokens::is_value_with_type(t, config_value::type::STRING)) {
                throw parse_error(string_format("expecting a quoted string inside file(), classpath(), or url(), rather than %s", t->to_string().c_str()));
            }
            ...
            if (t->token_text() != ")") {
                throw parse_error(string_format("expecting a close parentheses ')' here, not: %s", t->to_string().c_str()));
            }
            ...
            throw parse_error(string_format("include keyword is not followed by a quoted string, but by: %s", t->to_string().c_str()));
        }
    }
        ...
        while (true) {
            ...
            if (t->get_token_type() == token_type::CLOSE_CURLY) {
                ...
                if (_flavor == config_syntax::CONF && after_key->get_token_type() == token_type::OPEN_CURLY) {
                    ...
                    if (!is_key_value_separator(after_key)) {
                        ...
                        string_format("Key '%s' may not be followed by token: %s", key_path->render().c_str(), after_key->to_string().c_str())));
                    }
                ...
                if (remaining.empty()) {
                    ...
                    if (!emplace.second) {
                        ...
                        if (_flavor == config_syntax::JSON) {
                            throw parse_error(string_format("JSON does not allow duplicate fields: '%s' was already seen", key.c_str()));
                        }
            ...
            if (check_element_separator(object_nodes)) {
                ...
                if (t->get_token_type() == token_type::CLOSE_CURLY) {
                    ...
                    throw parse_error(add_quote_suggestion(t->to_string(),
                         string_format("Expecting close brace '}' or a comma, got %s", t->to_string().c_str()), last_inside_equals, last_path));
                } else {
                    if (t->get_token_type() == token_type::END) {
                        ...
                        throw parse_error(add_quote_suggestion(t->to_string(),
                            string_format("Expecting end of input or a comma, got %s", t->to_string().c_str()), last_inside_equals, last_path));
                    }
                }
            }
        }
    ...
    shared_ptr<config_node_complex_value> parse_context::parse_array() {
        ...
        if (next_value) {
            ...
            if (t->get_token_type() == token_type::CLOSE_SQUARE) {
                ...
                throw parse_error(string_format("List should have ']' or a first element after the '[', instead had token: %s (if you want %s to be part of a string value, then double quote it)",  t->to_string().c_str(), t->to_string().c_str()));
            }
        }
        ...
        while (true) {
            ...
            if (check_element_separator(children)) {
                ...
                if (t->get_token_type() == token_type::CLOSE_SQUARE) {
                    ...
                    throw parse_error(string_format("List should have ended with ']' or had a comma, instead had token: %s (if you want %s to be part of a string value, then double quote it)", t->to_string().c_str(), t->to_string().c_str()));
                }
            }
            ...
            if (next_value) {
                ...
                if (is_valid_array_element(t)) {
                    ...
                    throw parse_error(string_format("List should have had a new element after a comma, instead had token: %s (if you want the comma or %s to be part of a string value, then double quote it)", t->to_string().c_str(), t->to_string().c_str()));
                }
            }
        }
    }
    ...
    shared_ptr<config_node_root> parse_context::parse() {
        ...
        if (t->get_token_type() == token_type::START) {
            ...
            throw config_exception(string_format("token stream did not begin with START, had %s", t->to_string().c_str()));
        }
        ...
        if (t->get_token_type() == token_type::OPEN_CURLY || t->get_token_type() == token_type::OPEN_SQUARE) {
            ...
            if (_flavor == config_syntax::JSON) {
                if (t->get_token_type() == token_type::END) {
                    ...
                    throw parse_error(string_format("Document must have an object or array at root, unexpected token: %s", t->to_string().c_str()));
                }
            } else {
        ...
        if (t->get_token_type() == token_type::END) {
            ...
            throw parse_error(string_format("Document has trailing tokens after first object or array: %s", t->to_string().c_str()));
        }
    }
