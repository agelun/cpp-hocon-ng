        ...
        shared_value v = [&]() -> shared_value {
            if (auto val = dynamic_pointer_cast<const config_node_simple_value>(n)) {
                ...
                throw parse_exception(*line_origin(),
                                      string_format("Expecting a value but got wrong node type: %s", typeid(deref_n).name()));
            }
        }();
            ...
            if (auto comment = dynamic_pointer_cast<const config_node_comment>(node)) {
                ...
                if (remaining.empty()) {
                    ...
                    if (existing != values.end()) {
                        ...
                        if (_flavor == config_syntax::JSON) {
                            throw parse_exception(*line_origin(), string_format("JSON does not allow duplicate fields: '%s' was already seen at %s", key->c_str(), existing->second->origin()->description().c_str()));
                        } else {
